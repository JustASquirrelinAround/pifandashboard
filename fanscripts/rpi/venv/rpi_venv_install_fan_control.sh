...
echo "Installing required Python packages and setting up virtual environment..."
apt update
apt install -y python3-venv

# Create venv if it doesn't exist
if [ ! -d "$HOME/pifandashboard/venv" ]; then
  python3 -m venv $HOME/pifandashboard/venv
fi

# Activate venv and install RPi.GPIO
source $HOME/pifandashboard/venv/bin/activate
...
[Service]
Type=simple
ExecStart=$HOME/pifandashboard/venv/bin/python $HOME/pifandashboard/FanProportional.py
Restart=always
User=root
